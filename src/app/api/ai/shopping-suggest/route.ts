...
import {
  buildShoppingSuggestSystemInstruction,
...
export async function POST(req: Request) {
  ...
  const { meal, available_ingredients, health_profile } = parsed.data;
  const hp = health_profile as HealthProfilePayload;
  const system = buildShoppingSuggestSystemInstruction(hp);
  const userMsg = buildShoppingSuggestUserMessage({
    meal,
  ...
  try {
    ...
    let out = parseShoppingSuggestJson(raw);
    if (!out.success) {
      raw = await generateGeminiJson(system, userMsg + GEMINI_JSON_RETRY_SUFFIX);
      out = parseShoppingSuggestJson(raw);
    }
    if (!out.success) {
      return NextResponse.json(
